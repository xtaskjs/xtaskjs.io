...
const esTranslations: Record<string, string> = {
  ...
  "Value Objects And DTO Transformation": "Value objects y transformación de DTO",
  "DTO-oriented decorators from @xtaskjs/value-objects that turn raw request fields into normalized domain wrappers before controllers and services consume them.": "Decoradores orientados a DTO de @xtaskjs/value-objects que convierten campos crudos de la petición en envoltorios de dominio normalizados antes de que los consuman controladores y servicios.",
  "Transforms raw DTO input into a value object instance during class-transformer conversion so downstream code receives a validated wrapper instead of a primitive.": "Transforma entrada cruda de DTO en una instancia de value object durante la conversión con class-transformer para que el código posterior reciba un envoltorio validado en lugar de una primitiva.",
  "Normalize DTO email input": "Normalizar email DTO de entrada",
  ...
  "Cache Manifest": "Manifiesto en caché",
  "On first boot the kernel scans src/ and writes .xtask-manifest.json. Subsequent starts load the manifest directly, skipping the filesystem scan entirely and reducing startup time by 40–60%.": "En el primer arranque el kernel analiza src/ y escribe .xtask-manifest.json. Los arranques posteriores cargan el manifiesto directamente, omitiendo el escaneo del sistema de archivos y reduciendo el tiempo de arranque un 40–60%.",
  "Prebuilt Manifest": "Manifiesto precompilado",
  "Running npm run build generates .xtask-manifest.prebuilt.json at compile time. Production starts load this file first, giving the fastest possible boot without any scanning.": "npm run build genera .xtask-manifest.prebuilt.json en tiempo de compilación. Los arranques en producción cargan este archivo primero, logrando el arranque más rápido posible sin ningún escaneo.",
  "Pool Imports Async": "Importaciones en paralelo (Pool Async)",
  "Discovered files are imported through a bounded semaphore pool. XTASK_IMPORT_CONCURRENCY (default 10) limits parallel imports to prevent filesystem saturation. Tune to 16–24 for larger apps.": "Los archivos descubiertos se importan a través de un pool de semáforos acotado. XTASK_IMPORT_CONCURRENCY (por defecto 10) limita las importaciones en paralelo para evitar la saturación del sistema de archivos. Ajústalo a 16–24 para aplicaciones más grandes.",
  "Lazy DI Resolution": "Resolución DI perezosa",
  "Constructor-injected dependencies are wrapped in transparent proxies and only instantiated on first access. Startup avoids creating services that are never called, reducing boot time for optional integrations.": "Las dependencias inyectadas por constructor se envuelven en proxies transparentes y solo se instancian en el primer acceso. El arranque evita crear servicios que nunca se llaman, reduciendo el tiempo de inicio para integraciones opcionales.",
  "Hot Manifest Watcher": "Watcher de manifiesto en caliente",
  "In development, a file watcher applies incremental manifest updates. Changed files are re-imported and re-registered in the container without restarting the process.": "En desarrollo, un observador de archivos aplica actualizaciones incrementales al manifiesto. Los archivos modificados se reimportan y se vuelven a registrar en el contenedor sin reiniciar el proceso.",
  ...
  "Transport-agnostic queue handlers, broker helpers, publish decorators, and lifecycle-managed consumers.": "Handlers de colas agnósticos del transporte, utilidades para brokers, decoradores de publicación y consumidores gestionados por el ciclo de vida.",
  "Queues brings broker-backed and in-memory message delivery into xtaskjs. It registers named transports, discovers decorated consumers from DI-managed services, exposes injectable producer APIs, and coordinates retries, dead-letter routing, and consumer start-stop behavior through the application lifecycle.": "Queues incorpora entrega de mensajes con broker y en memoria a xtaskjs. Registra transports con nombre, descubre consumidores decorados desde servicios gestionados por DI, expone APIs de productores inyectables y coordina reintentos, dead-letter routing y el arranque o parada de consumidores mediante el ciclo de vida de la aplicación.",
  "configureQueues() sets default transport behavior and can disable the automatic in-memory transport.": "configureQueues() establece el comportamiento por defecto del transporte y puede desactivar el transporte automático en memoria.",
  "registerQueueTransport(), registerInMemoryQueueTransport(), createRabbitMqTransport(), and createMqttTransport() connect brokers or local transports.": "registerQueueTransport(), registerInMemoryQueueTransport(), createRabbitMqTransport() y createMqttTransport() conectan brokers o transports locales.",
  "QueueHandler(), QueuePattern(), PublishToQueue(), InjectQueueService(), and InjectQueueTransport() cover consumer, producer, and transport-injection flows.": "QueueHandler(), QueuePattern(), PublishToQueue(), InjectQueueService() e InjectQueueTransport() cubren flujos de consumidores, productores e inyección de transports.",
  "QueueService can publish messages, create producers, inspect consumers and groups, and start or stop queue runtime state through lifecycle-managed APIs.": "QueueService puede publicar mensajes, crear productores, inspeccionar consumidores y grupos, e iniciar o detener el estado del runtime de colas mediante APIs gestionadas por el ciclo de vida.",
  "Initialized automatically by @xtaskjs/core when the package is installed and queue configuration or decorated consumers are present.": "Se inicializa automáticamente mediante @xtaskjs/core cuando el paquete está instalado y existe configuración de colas o consumidores decorados.",
  "Pairs with the built-in in-memory transport for local development and tests, and with RabbitMQ or MQTT helpers for broker-backed delivery.": "Se combina con el transporte integrado en memoria para desarrollo local y tests, y con las utilidades de RabbitMQ o MQTT para entrega apoyada en brokers.",
  "Demonstrated by the 16-queues_memory_app and 17-queues_rabbitmq_app samples.": "Lo demuestran los ejemplos 16-queues_memory_app y 17-queues_rabbitmq_app.",
  "Named transport plus decorated queue consumers": "Transport con nombre y consumidores de colas decorados",
  "Introduces the queues package with the built-in in-memory transport, decorated consumers, pattern listeners, and publish-on-return workflows.": "Introduce el paquete queues con el transporte integrado en memoria, consumidores decorados, listeners por patrones y flujos de publicación al devolver resultados.",
  "Configure configureQueues() with an explicit memory transport and registerInMemoryQueueTransport() before CreateApplication().": "Configura configureQueues() con un transporte memory explícito y registerInMemoryQueueTransport() antes de CreateApplication().",
  "POST to /queues/orders to publish a message and watch QueueHandler and QueuePattern consumers update the in-memory event log.": "Haz POST a /queues/orders para publicar un mensaje y ver cómo los consumidores QueueHandler y QueuePattern actualizan el registro de eventos en memoria.",
  "POST to /queues/orders/:id/complete to exercise PublishToQueue() and observe chained local queue workflows.": "Haz POST a /queues/orders/:id/complete para probar PublishToQueue() y observar flujos locales de colas encadenados.",
  "Moves the same queue model to RabbitMQ, adding topic routing, competing consumers, retries, and dead-letter handling through the broker helper.": "Traslada el mismo modelo de colas a RabbitMQ, añadiendo routing por topics, consumidores en competencia, reintentos y gestión de dead-letter mediante la utilidad del broker.",
  "Configure the default transport with createRabbitMqTransport() and registerQueueTransport() before CreateApplication().": "Configura el transporte por defecto con createRabbitMqTransport() y registerQueueTransport() antes de CreateApplication().",
  "Publish orders through /queues/orders and /queues/orders/:id/fail to inspect retries, audit listeners, and dead-letter routing.": "Publica pedidos mediante /queues/orders y /queues/orders/:id/fail para inspeccionar reintentos, listeners de auditoría y el dead-letter routing.",
  "Open /queues/status to inspect started consumers, transport metadata, groups, retry settings, and recent events.": "Abre /queues/status para inspeccionar consumidores iniciados, metadata de transports, grupos, ajustes de reintento y eventos recientes.",
  "Configuration and transports": "Configuración y transports",
  "Decorators and injectors": "Decoradores e inyectores",
  "Runtime service and lifecycle": "Servicio de runtime y ciclo de vida",
  "Configure queue defaults, register transports, and decorate consumers while modules load so the queue runtime knows which producers and handlers to wire.": "Configura valores por defecto de colas, registra transports y decora consumidores mientras cargan los módulos para que el runtime de colas sepa qué productores y handlers conectar.",
  "The queues lifecycle publishes QueueService and named transport tokens into the container, discovers decorated consumers, and prepares them for lifecycle-ready startup.": "El ciclo de vida de queues publica QueueService y tokens de transport con nombre en el contenedor, descubre consumidores decorados y los prepara para arrancar en la fase ready del ciclo de vida.",
  "Active consumers are stopped and connected transports are disconnected automatically so broker resources shut down with the application.": "Los consumidores activos se detienen y los transports conectados se desconectan automáticamente para que los recursos del broker se apaguen junto con la aplicación.",
  "1. Configure transports and defaults": "1. Configura transports y valores por defecto",
  "Start with the built-in in-memory transport for local workflows, or register RabbitMQ or MQTT helpers when messages must cross process boundaries.": "Empieza con el transporte integrado en memoria para flujos locales, o registra las utilidades de RabbitMQ o MQTT cuando los mensajes deban cruzar límites de proceso.",
  "2. Decorate consumers and publishers": "2. Decora consumidores y publicadores",
  "Use QueueHandler and QueuePattern for consumers, PublishToQueue for method-result events, and InjectQueueService when services need direct publish or producer APIs.": "Usa QueueHandler y QueuePattern para consumidores, PublishToQueue para eventos basados en el resultado del método e InjectQueueService cuando los servicios necesiten APIs directas de publicación o productores.",
  "3. Inspect and control the queue runtime": "3. Inspecciona y controla el runtime de colas",
  "Use QueueService to list groups and consumers, create producers with defaults, and start or stop queue processing for diagnostics or operations endpoints.": "Usa QueueService para listar grupos y consumidores, crear productores con valores por defecto e iniciar o detener el procesamiento de colas para endpoints de diagnóstico u operaciones.",
  "Value object primitives, conversion helpers, DTO decorators, and DI factory integration helpers.": "Primitivas de value objects, utilidades de conversión, decoradores para DTO y helpers de integración con fábricas DI.",
  "Value objects gives xtaskjs projects a consistent way to model domain primitives and structured payload wrappers. It standardizes conversion from raw input, JSON strings, and serialized payloads while optionally bridging DTO transformation and DI-managed factory creation.": "Value objects ofrece a los proyectos xtaskjs una forma coherente de modelar primitivas de dominio y envoltorios de payload estructurados. Estandariza la conversión desde entrada cruda, cadenas JSON y payloads serializados, y además puede enlazar la transformación de DTO con la creación de fábricas gestionadas por DI.",
  "Base classes cover string, number, boolean, bigint, date, and JSON-backed value objects.": "Las clases base cubren value objects respaldados por string, number, boolean, bigint, date y JSON.",
  "Conversion helpers normalize raw values, JSON strings, and serialized payloads into predictable wrappers.": "Las utilidades de conversión normalizan valores crudos, cadenas JSON y payloads serializados en envoltorios predecibles.",
  "TransformValueObject() plugs value objects into class-transformer DTO pipelines.": "TransformValueObject() integra los value objects en las canalizaciones DTO de class-transformer.",
  "createValueObjectFactory(), InjectableValueObjectFactory, and ValueObjectFactoryFor() support container-managed creation flows.": "createValueObjectFactory(), InjectableValueObjectFactory y ValueObjectFactoryFor() permiten flujos de creación gestionados por el contenedor.",
  "Fits naturally with @xtaskjs/common ValidationPipe once DTOs are transformed through class-transformer.": "Encaja de forma natural con ValidationPipe de @xtaskjs/common una vez que los DTO se transforman mediante class-transformer.",
  "Can register DI-friendly factories through @xtaskjs/core when value object creation belongs in services.": "Puede registrar fábricas compatibles con DI a través de @xtaskjs/core cuando la creación de value objects pertenece a servicios.",
  "Useful in HTTP DTOs, domain models, and persistence boundaries where primitive normalization must stay explicit.": "Resulta útil en DTO HTTP, modelos de dominio y límites de persistencia donde la normalización de primitivas debe seguir siendo explícita.",
  "DTO transformation plus DI factory registration": "Transformación de DTO más registro de fábrica DI",
  "Domain modeling": "Modelado de dominio",
  "Serialization": "Serialización",
  "DTO pipelines": "Canalizaciones DTO",
  "Import reflect-metadata before defining value objects that rely on decorator-driven DTO or DI integrations.": "Importa reflect-metadata antes de definir value objects que dependan de integraciones DTO o DI basadas en decoradores.",
  "The package itself does not register a lifecycle manager, but ValueObjectFactoryFor() can register DI-ready factories through @xtaskjs/core during module loading.": "El paquete en sí no registra un gestor de ciclo de vida, pero ValueObjectFactoryFor() puede registrar fábricas preparadas para DI a través de @xtaskjs/core durante la carga del módulo.",
  "Value objects are pure wrappers with no owned resources, so shutdown usually means letting container-managed factories disappear with the rest of the app.": "Los value objects son envoltorios puros sin recursos propios, así que el apagado normalmente consiste en dejar que las fábricas gestionadas por el contenedor desaparezcan junto con el resto de la aplicación.",
  "1. Define value object classes": "1. Define clases de value objects",
  "Extend StringValueObject, NumberValueObject, DateValueObject, or JsonValueObject and normalize invariants inside the constructor.": "Extiende StringValueObject, NumberValueObject, DateValueObject o JsonValueObject y normaliza las invariantes dentro del constructor.",
  "2. Transform DTO input": "2. Transforma la entrada DTO",
  "Use TransformValueObject() on DTO fields when request payloads should become domain-friendly wrappers during class-transformer conversion.": "Usa TransformValueObject() en campos DTO cuando los payloads de petición deban convertirse en envoltorios cómodos para el dominio durante la conversión con class-transformer.",
  "3. Add factories when needed": "3. Añade fábricas cuando haga falta",
  "Create ad hoc factories with createValueObjectFactory() or register InjectableValueObjectFactory implementations when services should resolve them from the container.": "Crea fábricas ad hoc con createValueObjectFactory() o registra implementaciones de InjectableValueObjectFactory cuando los servicios deban resolverlas desde el contenedor.",
  "Types and contracts": "Tipos y contratos",
  "Conversion helpers": "Utilidades de conversión",
  "Base value objects": "Value objects base",
  "Factory helpers": "Utilidades de fábrica",
  "DTO decorators": "Decoradores DTO",
  "Queues And Messaging": "Colas y mensajería",
  "Queue consumer, publish, and injector decorators exported by @xtaskjs/queues. These wire broker transports and in-memory queues into DI-managed services.": "Decoradores de consumo, publicación e inyección de colas exportados por @xtaskjs/queues. Conectan transports de broker y colas en memoria con servicios gestionados por DI.",
  "Registers a named queue consumer for a concrete queue and supports retries, groups, dead-letter routing, and transport selection.": "Registra un consumidor de colas con nombre para una cola concreta y admite reintentos, grupos, dead-letter routing y selección de transporte.",
  "Queue-specific consumer": "Consumidor específico de cola",
  "Registers a topic or pattern-based listener that can subscribe to wildcards or broker topic filters depending on the transport.": "Registra un listener basado en topic o patrón que puede suscribirse a comodines o filtros de topics del broker según el transporte.",
  "Pattern listener": "Listener por patrón",
  "Publishes the resolved method result to a queue after the method completes while preserving the original return value.": "Publica en una cola el resultado resuelto del método una vez que termina, conservando el valor de retorno original.",
  "Publish method result": "Publicar resultado del método",
  "Injects QueueService so services or controllers can publish messages, create producers, and inspect runtime consumers and transports.": "Inyecta QueueService para que servicios o controladores puedan publicar mensajes, crear productores e inspeccionar consumidores y transports del runtime.",
  "Inject queue service": "Inyectar servicio de colas",
  "Injects a named QueueTransport implementation when low-level broker publish or subscription control is required.": "Inyecta una implementación de QueueTransport con nombre cuando se necesita publicación o control de suscripción a bajo nivel del broker.",
  "Inject named queue transport": "Inyectar transporte de colas con nombre",
  ...
  "Detailed reference for core, common, adapters, TypeORM, security, mailer, internationalization, scheduler, cache, queues, socket-io, and value objects.": "Referencia detallada de core, common, adaptadores, TypeORM, security, mailer, internationalization, scheduler, cache, queues, socket-io y value objects.",
  "Socket.IO gateways, event decorators, injectable realtime services, and lifecycle-managed server attachment.": "Gateways de Socket.IO, decoradores de eventos, servicios realtime inyectables y acoplamiento al servidor gestionado por el ciclo de vida.",
  "Socket.IO integrates realtime messaging into xtaskjs without leaving the framework's DI and lifecycle model. It discovers decorated gateway services, attaches to the live HTTP server during app.listen(), exposes namespaces and broadcast helpers through DI, and tears down cleanly during app.close().": "Socket.IO integra mensajería realtime en xtaskjs sin salir del modelo de DI y ciclo de vida del framework. Descubre servicios gateway decorados, se conecta al servidor HTTP activo durante app.listen(), expone namespaces y utilidades de broadcast mediante DI y se desmonta limpiamente durante app.close().",
  "SocketGateway(), OnSocketConnection(), OnSocketDisconnect(), and OnSocketEvent() declare realtime handlers on regular @Service() classes.": "SocketGateway(), OnSocketConnection(), OnSocketDisconnect() y OnSocketEvent() declaran handlers realtime sobre clases @Service() normales.",
  "Returned values are sent through Socket.IO acknowledgements automatically when the client provides an ack callback.": "Los valores devueltos se envían automáticamente mediante acknowledgements de Socket.IO cuando el cliente proporciona un callback ack.",
  "InjectSocketService(), InjectSocketServer(), InjectSocketNamespace(), and InjectSocketLifecycleManager() expose the runtime through DI.": "InjectSocketService(), InjectSocketServer(), InjectSocketNamespace() e InjectSocketLifecycleManager() exponen el runtime mediante DI.",
  "SocketIoService can broadcast events, list gateways, inspect namespaces, and reuse the lifecycle-managed Socket.IO server.": "SocketIoService puede emitir eventos, listar gateways, inspeccionar namespaces y reutilizar el servidor Socket.IO gestionado por el ciclo de vida.",
  "Initialized automatically by @xtaskjs/core when the package is installed and the app starts listening on an HTTP adapter.": "Se inicializa automáticamente mediante @xtaskjs/core cuando el paquete está instalado y la aplicación empieza a escuchar sobre un adaptador HTTP.",
  "Attaches to the live server behind node-http, Express, or Fastify without changing controller or service registration patterns.": "Se conecta al servidor activo detrás de node-http, Express o Fastify sin cambiar los patrones de registro de controladores o servicios.",
  "Demonstrated by the 23-socket_io_express_app sample with room joins, acknowledgements, and HTTP-triggered announcements.": "Lo demuestra el ejemplo 23-socket_io_express_app con uniones a salas, acknowledgements y anuncios disparados por HTTP.",
  "Decorated gateway plus service broadcast": "Gateway decorado más broadcast desde servicio",
  "Express sample application using decorated Socket.IO gateways, room joins, automatic acknowledgements, and HTTP-triggered broadcasts.": "Aplicación de ejemplo con Express que usa gateways Socket.IO decorados, uniones a salas, acknowledgements automáticos y broadcasts disparados por HTTP.",
  "Boot Express through ExpressAdapter and let @xtaskjs/socket-io attach to the live HTTP server during listen().": "Arranca Express mediante ExpressAdapter y deja que @xtaskjs/socket-io se conecte al servidor HTTP activo durante listen().",
  "Open the demo page, join the /chat namespace lobby, and emit chat.join plus chat.message events from the browser client.": "Abre la página de demostración, únete al lobby del namespace /chat y emite eventos chat.join y chat.message desde el cliente del navegador.",
  "Call /socket/announce/:message to publish a server.announcement event from an HTTP controller through InjectSocketService().": "Llama a /socket/announce/:message para publicar un evento server.announcement desde un controlador HTTP mediante InjectSocketService().",
  "Configure Socket.IO defaults when needed and decorate DI-managed services with SocketGateway plus socket handler decorators so xtaskjs can discover them.": "Configura los valores por defecto de Socket.IO cuando haga falta y decora servicios gestionados por DI con SocketGateway y decoradores de handlers de socket para que xtaskjs pueda descubrirlos.",
  "During app.listen()": "Durante app.listen()",
  "The integration resolves the live HTTP server from the selected adapter, creates the Socket.IO server, registers namespace tokens, and binds discovered gateways.": "La integración resuelve el servidor HTTP activo desde el adaptador seleccionado, crea el servidor Socket.IO, registra tokens de namespace y enlaza los gateways descubiertos.",
  "The Socket.IO server is closed before the HTTP adapter and container are torn down so namespaces, rooms, and subscriptions stop cleanly.": "El servidor Socket.IO se cierra antes de desmontar el adaptador HTTP y el contenedor para que namespaces, salas y suscripciones se detengan limpiamente.",
  "1. Configure namespace defaults": "1. Configura namespaces por defecto",
  "Call configureSocketIo() when you need a non-root default namespace or custom Socket.IO server options such as CORS and transport settings.": "Llama a configureSocketIo() cuando necesites un namespace por defecto distinto de la raíz o opciones personalizadas del servidor Socket.IO, como CORS y ajustes de transporte.",
  "2. Decorate gateways and events": "2. Decora gateways y eventos",
  "Use SocketGateway on a regular @Service() class, then add OnSocketConnection, OnSocketDisconnect, and OnSocketEvent handlers for each namespace workflow.": "Usa SocketGateway sobre una clase @Service() normal y después añade handlers OnSocketConnection, OnSocketDisconnect y OnSocketEvent para cada flujo del namespace.",
  "3. Broadcast from the rest of the app": "3. Emite desde el resto de la app",
  "Inject SocketIoService, a namespace token, or the raw server when controllers and services need to emit announcements or inspect realtime runtime state.": "Inyecta SocketIoService, un token de namespace o el servidor bruto cuando controladores y servicios necesiten emitir anuncios o inspeccionar el estado realtime del runtime.",
  "Socket.IO Realtime Gateways": "Gateways realtime de Socket.IO",
  "Gateway, event, and injector decorators exported by @xtaskjs/socket-io. These keep realtime handlers inside regular xtaskjs services and wire Socket.IO namespaces into the DI container.": "Decoradores de gateway, eventos e inyección exportados por @xtaskjs/socket-io. Mantienen los handlers realtime dentro de servicios xtaskjs normales y conectan namespaces de Socket.IO al contenedor DI.",
  "Marks a DI-managed service as a realtime gateway, assigning namespace, optional name, groups, and disabled state for lifecycle discovery.": "Marca un servicio gestionado por DI como gateway realtime, asignando namespace, nombre opcional, grupos y estado disabled para el descubrimiento del ciclo de vida.",
  "Decorated realtime gateway": "Gateway realtime decorado",
  "Runs when a client connects to the gateway namespace and receives the connected socket plus handler context.": "Se ejecuta cuando un cliente se conecta al namespace del gateway y recibe el socket conectado junto con el contexto del handler.",
  "Track client connections": "Registrar conexiones de clientes",
  "Runs when Socket.IO disconnects a client, making it a good place to update presence, release room state, or log disconnect reasons.": "Se ejecuta cuando Socket.IO desconecta a un cliente, por lo que es un buen lugar para actualizar presencia, liberar estado de salas o registrar motivos de desconexión.",
  "Handle disconnect reasons": "Gestionar motivos de desconexión",
  "Registers a named Socket.IO event handler, optionally overrides namespace or handler options, and automatically acknowledges returned values when the client expects an ack.": "Registra un handler de evento Socket.IO con nombre, puede sobrescribir el namespace o las opciones del handler y reconoce automáticamente los valores devueltos cuando el cliente espera un ack.",
  "Named chat event with acknowledgement": "Evento de chat con acknowledgement",
  "Injects SocketIoService so controllers and services can emit events, inspect namespaces, and list discovered gateways.": "Inyecta SocketIoService para que controladores y servicios puedan emitir eventos, inspeccionar namespaces y listar gateways descubiertos.",
  "Inject the socket helper service": "Inyectar el servicio helper de sockets",
  "Injects the SocketIoLifecycleManager for lower-level diagnostics, gateway inspection, namespace access, or manual emit control.": "Inyecta SocketIoLifecycleManager para diagnósticos de bajo nivel, inspección de gateways, acceso a namespaces o control manual de emisiones.",
  "Inject the socket lifecycle manager": "Inyectar el gestor de ciclo de vida de sockets",
  "Injects the root Socket.IO server instance when low-level adapter APIs or global broadcasts are required.": "Inyecta la instancia raíz del servidor Socket.IO cuando se necesitan APIs de adaptador de bajo nivel o broadcasts globales.",
  "Inject the root Socket.IO server": "Inyectar el servidor raíz de Socket.IO",
  "Injects one namespace instance by name so services can target a specific room topology without routing every emit through the root server.": "Inyecta una instancia de namespace por nombre para que los servicios puedan dirigirse a una topología de salas concreta sin enrutar cada emisión a través del servidor raíz.",
  "Inject a named namespace": "Inyectar un namespace con nombre",
  "Configuration and tokens": "Configuración y tokens",
  "Gateways and injectors": "Gateways e inyectores",
  "Runtime service and types": "Servicio de runtime y tipos",
};
